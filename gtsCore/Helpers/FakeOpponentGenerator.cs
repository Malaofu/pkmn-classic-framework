...
public class FakeOpponentGenerator
{
    ...
    /// <returns></returns>
    public BattleTowerRecordBase[] GenerateFakeOpponents(FakeOpponentFactory factory, int count)
    {
        if (count == 0) return [];
        ...
        var results = values.DrawWithoutReplacement(rand).Select(i => GenerateFakeOpponent(factory, _pokedex, i));

        return results.ToArray();
    }

    public static BattleTowerRecordBase GenerateFakeOpponent(FakeOpponentFactory factory, Pokedex pokedex, int index)
    {
...
{
    public override BattleTowerRecordBase CreateRecord(Pokedex pokedex)
    {
        BattleTowerRecord4 record = new(pokedex)
        {
...
public class FakeOpponentFactory5 : FakeOpponentFactory
{
    public override BattleTowerRecordBase CreateRecord(Pokedex pokedex)
    {
        BattleSubwayRecord5 record = new(pokedex)
        {
